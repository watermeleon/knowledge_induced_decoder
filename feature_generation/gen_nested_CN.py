...
def get_singular(token):
    """returns:
    the singular form of a token,
    if not plural or no singular form of plural: return False
    """
    tokens = nlp(token)
...
def get_plurals(openwords_set):
    ...
    for plur_word in total_plural:
        ...
        if word_sing in words_sing:
            print(
                "singular already exists", word_sing, plur_word, sing_to_plur[word_sing]
            )
...
def remove_duplicate_rc(lookupdict):
    """removes all edges for duplicate concepts"""
    lookupdict2 = {}
...
def parse_rel_label(lookupdict):
    """the relationship label in the token appropriate way"""
...
def main(openweb10th_path, ConcNet_Eng_path, out_path, args):
    ...
    for i in tqdm(range(tot_csv)):
        ...
        # skip if the concepts have the same stem
        ...
        # if conc_one is bird, and plural is birds, an : append idx to birds
        if (
            plur1 in total_plural
            and plur1 != conc_one
            and conc_two not in openwords_set
        ):
            concNet_nested_filt[plur1].append([rel, conc_two, 0])
        if (
            plur2 in total_plural
            and plur2 != conc_two
            and conc_one not in openwords_set
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--openweb10th_path", default="../../data_files/openwebtext_10th.csv", type=str
    )
    parser.add_argument(
        "--ConcNet_Eng_path", default="../../data_files/ass_onlyenglish.csv", type=str
    )
    parser.add_argument(
        "--out_path",
        default="../../data_files/CN_feats/conceptnet_filt_nest_labels_new.pkl",
        type=str,
    )
