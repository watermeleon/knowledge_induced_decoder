...
class TextField(RawField):
    ...
    def numericalize(self, arr, device=None):
        ...
        if self.use_vocab:
            ...
            var = torch.tensor(arr, dtype=self.dtype, device=device)
        elif self.transform_tok is not None:
                ...
                # due to bert tokenization make each sent in minibatch same length
                maxlen = max([len(sent) for sent in arr])
                newarr = []
                for sent in arr:
                    diff = maxlen - len(sent)
                    if diff >0:
                        sent = sent + [self.transform_pad_id]*diff
                    newarr.append(sent)
                arr = newarr
                var = torch.tensor(arr, dtype=self.dtype, device=device)
        else:
